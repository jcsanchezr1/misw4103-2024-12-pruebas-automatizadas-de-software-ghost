...
4. Antes de ejecutar los reportes con kraken instalar las dependencias requeridas para ejecución de pruebas de datos aleatorios y pseudo aleatorios (`faker` y `axios`)

`npm install`

5. Ejecutar el siguiente comando para ejeuctar las pruebas, indicando la ruta relativa de la librería que se instala en el directorio node_modules. Es decir, al momento de ejecutar la prueba, el comando debe ser:

`./node_modules/kraken-node/bin/kraken-node run`

6. Validar los reportes y resultados de la prueba en:
...
2. Antes de ejecutar los reportes con cypress instalar las dependencias requeridas para ejecución de pruebas de datos aleatorios (`faker`)

`npm install`

3. Para ejecutar las pruebas de Cypress abra una terminal y ejecute el siguiente comando:

`cypress open`

4. Desde la UI de Cypress añadir el proyecto seleccionado (browse manually), seleccionar la ruta del repositorio clonado incluyendo la carpeta cypress `/misw4103-2024-12-pruebas-automatizadas-de-software-ghost/cypress`
...
6. Seleccionar navegador (se recomienda usar `Chrome`) y posteriormente hacer click en `Start E2E Testing in Chrome`
...
7. Una vez se ejcuta el paso 5 se abre un navegador `Chrome` para ejecutar las pruebas, en donde debemos hacer click en `Specs` que es en donde se encuentran todos los escenarios implementados. 
...
8. Por último seleccionar cada uno de los archivos para ejecutar las pruebas de cada funcionalidad `member.cy.js` `page.cy.js` `post.cy.js` `profile.cy.js` `tag.cy.js`, luego de seleccionarlos se ejecutarán las pruebas y se deberá mostrar como en la siguiente imagen:
